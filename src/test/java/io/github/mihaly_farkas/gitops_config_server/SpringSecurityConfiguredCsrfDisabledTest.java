...
@ActiveProfiles({"public_github_repo", "encrypt_key", "spring_security_user", "no_csrf"})
class SpringSecurityConfiguredCsrfDisabledTest extends SpringSecurityTest {
  ...
  @DisplayName("POST requests without CSRF token are accepted when CSRF is disabled")
  @MethodSource("postRequests")
  @ParameterizedTest(name = "endpoint -> {0}")
  @SneakyThrows
  void csrfTokensAreNotRequiredForPosts(
      String endpoint,
    ...
    mockMvc
        .perform(
            post(endpoint)
                .with(httpBasic(springSecurityUsername(), springSecurityPassword()))
