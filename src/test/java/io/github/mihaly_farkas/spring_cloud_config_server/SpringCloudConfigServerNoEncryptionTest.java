...
class SpringCloudConfigServerNoEncryptionTest extends MockMvcTest {
  ...
  Stream<Arguments> configs() {
    return Stream.of(
        Arguments.of(
            configServerPrefix + "/spring_cloud_config_server-default.json",
            Map.of(
                "name", "Spring Cloud Config Server Example",
                "description", "Configuration for Spring Cloud Config Server Example",
                "invalid", Map.of("secret", "<n/a>"))),
        Arguments.of(
            configServerPrefix + "/spring_cloud_config_server-overrides_by_profile.json",
            Map.of(
                "name",
                "Spring Cloud Config Server Example (overridden by the 'overrides_by_profile' profile)",
                "description",
                "Configuration for Spring Cloud Config Server Example (overridden by the 'overrides_by_profile' profile)",
                "invalid",
                Map.of("secret", "<n/a>"))));
  }
