...
import static io.github.mihaly_farkas.spring_boot_config_server.lib.http.MediaType.APPLICATION_SPRING_BOOT_ACTUATOR_JSON;
...
abstract class SpringSecurityTest extends MockMvcTest {
  ...
  Stream<Arguments> actuatorEndpoints() {
    return Stream.concat(
        ...
        .map(path -> Arguments.of(path, OK, APPLICATION_SPRING_BOOT_ACTUATOR_JSON));
  }
  ...
  Stream<Arguments> springCloudConfigServerEndpoints() {
    return Stream.of(
        ...
        Arguments.of(
            configServerPrefix + "/spring_boot_config_server-default.json", OK, APPLICATION_JSON),
        Arguments.of(
