 ...
 * gitops-config-server.health.socket.enabled} being set to {@code true}. When enabled, it creates a
 * {@link HealthSocketServer} bean that listens on the configured Unix domain socket path for
 * incoming health check connections.
 *
...
public class HealthSocketServerConfig {
   ...
   * {@code gitops-config-server.health.socket.path} (defaults to {@code health.sock} in the current
   * working directory).
   *
